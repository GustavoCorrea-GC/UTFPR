...
void inicialista(lista *l)
{
    ...
    while (i < Num_City)
    {
        ...
        // printf("%c %d-",(char)(((int)'A')+i),((int)'A')+i);
        i++;
    }
    // printf("\n");
    /* no_lista *p=l->inicio;
    while(p->prox!=NULL){
        printf("p->arv=%c ", (char)p->Arv);
        p=p->prox;
    }
    printf("p->arv=%c ", (char)p->Arv);
    //printf("\n");
    ...
    while (l.quant != 0)
    {
        ...
        while (q->aresta != NULL)
        {
            if ((q->peso != 0) && (q->peso < valor) && (Busca_lista(&l, q->chave) == 1))
            {
                s = q;
                valor = q->peso;
                //printf("Trocou %c q->peso=%d\n",q->chave,q->peso);
            }
            //printf("Não Trocou %c q->peso=%d\n",q->chave,q->peso);
            q = q->aresta;
        }
        //printf("chave%c quant=%d inicio%c peso=%d\n",(char)s->chave,l.quant,(char)inicio,s->peso);
    ...
    for (i = 0; i < Num_City; i++)
    {
        for (j = 0; j < Num_City; j++)
        {
            if (j != i)
            {
                if (i < j)
                {
                    //printf("i=%d j=%d x1=%f y1=%f x2=%f y2=%f ", i, j, d[i][0], d[i][1], d[j][0], d[j][1]);
                    inserir_aresta(G, (int)'A' + i, (int)'A' + j, CalculaDist(d[i][0], d[i][1], d[j][0], d[j][1]), M);
                    //printf("Aresta %c e %c fim\n", (char)((int)'A' + i), (char)((int)'A' + j));
                }
            }
            //printf("opa %d", j);
        }
